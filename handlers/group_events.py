...
async def new_dsfcommands(message: types.Message):
    adm = ""
    if message.chat.type == 'private':
        pass
    else:
        admins_id = [(admin.user.id) for admin in await message.bot.get_chat_administrators(chat_id=message.chat.id)]
        admins_name = [(admin.user.full_name) for admin in await message.bot.get_chat_administrators(chat_id=message.chat.id)]
        admin_prefix = [(admin.custom_title) for admin in await message.bot.get_chat_administrators(chat_id=message.chat.id)]
        ...
        await message.answer('<b><i>Модераторы беседы</i></b>\n' + str(adm.replace('None', 'Модератор')), parse_mode='html')
